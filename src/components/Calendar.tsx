              ...
              <div className="grid gap-4">
                {events.map((event, i) => (
                  <motion.div
                    ...
                    className="group border border-border hover:border-accent/30 transition-all p-6 bg-brand-pale/10 flex flex-col md:flex-row md:items-center justify-between gap-6"
                  >
                    <div className="flex gap-6">
                      <div className="flex-shrink-0 w-16 h-16 border border-border flex flex-col items-center justify-center bg-bg group-hover:bg-accent group-hover:text-bg transition-colors">
                        <span className="urdu text-[10px] font-black uppercase tracking-tighter">
                          {new Date(event.date).toLocaleString('ur-PK', { month: 'short' })}
                        </span>
                        <span className="urdu text-2xl font-black leading-none">
                          {new Date(event.date).toLocaleString('ur-PK', { day: 'numeric' })}
                        </span>
                      </div>
                      <div>
                        <div className={`inline-flex px-2 py-0.5 border text-[8px] font-black uppercase tracking-widest mb-3 ${getTypeStyle(event.type)}`}>
                          {getTypeLabel(event.type)}
                        </div>
                        <h4 className="urdu text-xl text-ink font-bold group-hover:text-accent transition-colors">
                          {event.title}
                        </h4>
                        {event.description && (
                          <p className="urdu text-muted text-xs mt-2 font-medium max-w-md">
                            {event.description}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col items-end gap-2 text-right">
                       <div className="flex items-center gap-2 text-muted urdu text-[10px] font-bold tracking-widest">
                          <Calendar size={12} className="text-accent" />
                          <span>آغاز: {new Date(event.date).toLocaleDateString('ur-PK', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                       </div>
                       {event.endDate && (
                         <div className="flex items-center gap-2 text-muted urdu text-[10px] font-bold tracking-widest">
                            <Clock size={12} className="text-accent" />
                            <span>اختتام: {new Date(event.endDate).toLocaleDateString('ur-PK', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                         </div>
                       )}
                    </div>
                  </motion.div>
